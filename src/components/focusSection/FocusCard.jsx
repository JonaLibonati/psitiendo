...
export const FocusCard = ({ img, headline, text, index, ariaLabelOpen, ariaLabelClose, alt }) => {
  ...
  return (
    <li className="relative bg-charcoal h-[450px] w-full rounded-xl overflow-hidden shadow-lg cursor-pointer" onClick={() => setToggle(prev => !prev)}>
      <div className="relative size-full">
        <img src={img} className={`absolute size-full object-cover transition duration-800 ${toggle ? "opacity-0" : "opacity-100"}`} loading="lazy" alt={alt}/>
        <div className="absolute bg-charcoal z-30 size-full opacity-20"></div>
        <button className="absolute z-50 bg-white rounded-full right-0 bottom-0 m-4 shadow-lg cursor-pointer" aria-label={!toggle ? ariaLabelOpen : ariaLabelClose}>
          {
            toggle ?
